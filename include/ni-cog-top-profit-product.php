...
include_once("ni-cog-function.php"); 
	class Ni_COG_Top_Profit_Product extends Ni_COG_Function{
		...
		function get_top_product_query(){
			...
			$query .= "		,ROUND(SUM(ROUND(line_total.meta_value,2)-(ROUND(ni_cost_goods.meta_value,2)*(qty.meta_value))),2) as  product_profit";
			...
			$query .= " LEFT JOIN  {$wpdb->prefix}woocommerce_order_itemmeta as ni_cost_goods ON ni_cost_goods.order_item_id=order_items.order_item_id ";
				...
				if ("yesterday_top_product" == $product_type ){
					if ( $this->is_hpos_enable ){
						$query .= " AND   date_format( posts.date_created_gmt, '%Y-%m-%d') BETWEEN '{$yesterday}' AND '{$yesterday}'";
					}else{
				...
				if ("last_7_days_top_product" == $product_type ){
					if ( $this->is_hpos_enable ){
						$query .= " AND   date_format( posts.date_created_gmt, '%Y-%m-%d') BETWEEN '{$last_7_days}' AND '{$today}'";
					}else{
                 	...
                 	<tr>
                    	<?php foreach($columns  as $col_key=>$col_value): ?>
                        	<?php switch($col_key): case 1: break; ?>
								<?php case "line_total": ?>
                                <td> <?php  echo wc_price(  isset($row_value->$col_key)?$row_value->$col_key:0);?>  </td>
                            	<?php break; ?>

								<?php case "product_profit": ?>
                                <td> <?php  echo wc_price(  isset($row_value->$col_key)?$row_value->$col_key:0);?>  </td>
                            	<?php break; ?>		
