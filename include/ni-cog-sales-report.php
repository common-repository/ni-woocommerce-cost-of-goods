...
if( !class_exists( 'Ni_COG_Sales_Report' ) ) { 
	...
	class Ni_COG_Sales_Report  extends Ni_COG_Function{
		 ...
		 if ($this->is_hpos_enable){
			$order_country =  $this->get_order_country_hpos();
		 }else{
		...
		function get_order_country_hpos(){
			...
			$query .= "	order_addresses.country as 'billing_country'";
		
			$query .= "	FROM {$wpdb->prefix}wc_order_addresses as order_addresses	";	
			...
			//$query .= " AND billing_country.meta_key ='_billing_country' ";
			$query .= " GROUP BY order_addresses.country ";
			
			$query .= " Order BY order_addresses.country  ASC";	
		...
		function get_cog_table(){
		 ...
		 function get_query_data($type="DEFAULT")
		 {
			...
			if ( $this->is_hpos_enable ){	
				...
				$query .= ",order_addresses.first_name as billing_first_name ";
				$query .= ",order_addresses.last_name as billing_last_name ";
				$query .= ",order_addresses.email as billing_email ";
			...
			if ( $this->is_hpos_enable ){
				...
				if ($first_name  !=="" || $order_by ==='first_name'){
					$query .= "	AND order_addresses.first_name LIKE '%".$first_name ."%' ";
				}
				if ($last_name  !=="" || $order_by ==='last_name'){
					$query .= "	AND order_addresses.last_name LIKE '%".$last_name ."%' ";
				}

				if ($order_country  !=="" && $order_country  !=="-1"){
					$query .= "	AND order_addresses.country LIKE '%".$order_country ."%' ";
				}

				if ($email_address !==''){
					$query .= "	AND order_addresses.email LIKE '%".$email_address ."%' ";
				}
			...
			switch ($select_order) {
				...
				case "last_60_days":
					if ( $this->is_hpos_enable ){
						$query .= " AND  date_format( posts.date_created_gmt, '%Y-%m-%d') BETWEEN date_format(DATE_SUB(CURDATE(), INTERVAL 60 DAY), '%Y-%m-%d') AND   '{$today}' ";
					}else{
				...
				case "last_90_days":
					if ( $this->is_hpos_enable ){
						$query .= " AND  date_format( posts.date_created_gmt, '%Y-%m-%d') BETWEEN date_format(DATE_SUB(CURDATE(), INTERVAL 90 DAY), '%Y-%m-%d') AND   '{$today}' ";
					}else{
			...
			switch ($order_by) {
				...
				case "first_name":
					if ( $this->is_hpos_enable ){
						$query .= "  Order BY  order_addresses.first_name " .$sort;	
					}else{
				...
				case "last_name":
					if ( $this->is_hpos_enable ){
						$query .= "  Order BY  order_addresses.last_name " .$sort;	
					}else{
