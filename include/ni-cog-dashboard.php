	...
	class Ni_COG_Dashboard  extends Ni_COG_Function{
                        ...
                        <div class="row">
                           ...
                           <div class="col-xl-3 col-md-6 col-lg-4   box10">
							  <div class="card card-border-top card-border-top-box10  shadow p-3 mb-5 bg-white rounded">
                                  <div class="card-body card-body-padding">
                                  	<div class="card-block">
									<h6 class="m-b-20" style="font-size: 14px; text-transform: uppercase"><strong> <?php _e('Total Sales Order Count',  'wooreportcog'); ?></strong></h6>
									<p class="text-right"><span>&nbsp;</span></p>
						   ...
						   <div class="col-xl-3 col-md-6 col-lg-4   box1">
							  <div class="card card-border-top card-border-top-box1  shadow p-3 mb-5 bg-white rounded">
                                  <div class="card-body card-body-padding">
                                  	<div class="card-block">
									<h6 class="m-b-20" style="font-size: 14px; text-transform: uppercase"><strong><?php _e('This Year Sales Order Count',  'wooreportcog'); ?></strong></h6>
									<p class="text-right"><span>&nbsp;</span></p>
		 ...
		 function get_top_customer_query(){
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= " posts.billing_email as   billing_email ";
			}else{
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= ", order_addresses.email as   billing_email ";
				$cog_query .= ", order_addresses.first_name as   billing_first_name ";
				$cog_query .= ", order_addresses.last_name as   billing_last_name ";
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= " FROM {$wpdb->prefix}wc_orders as posts ";
			}else{
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= " LEFT JOIN {$wpdb->prefix}wc_order_addresses as order_addresses ON order_addresses.order_id = posts.ID ";
			}else{
			...
			$cog_query .= "	WHERE 1 = 1 ";
			if ( $this->is_hpos_enable ){
				$cog_query .= " AND posts.status NOT IN ('auto-draft','inherit')";
				$cog_query .= " AND posts.type ='shop_order'";
			}else{
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= " AND order_addresses.address_type = 'billing'";	 
			}else{
			...
			if ( $this->is_hpos_enable ){
				$cog_query .= " GROUP BY  posts.billing_email ";
			}else{
		 ...
		 function get_total_sales($period="CUSTOM",$start_date=NULL,$end_date=NULL){
			...
			if ($period =="YESTERDAY"){
				if ( $this->is_hpos_enable ){
					$query .= " AND   date_format( posts.date_created_gmt, '%Y-%m-%d') = DATE_SUB('$today_date', INTERVAL 1 DAY) "; 
				}else{
			...
			if ($period =="DAY"){		
				if ( $this->is_hpos_enable ){
					$query .= " AND   date_format( posts.date_created_gmt, '%Y-%m-%d') = '{$today_date}' "; 
      ...
      MONTH(date_format( posts.post_date, '%Y-%m-%d')) = MONTH(CURRENT_DATE()) ";
		...
		function get_profit_summary($period="CUSTOM",$start_date=NULL,$end_date=NULL){
			...
			if ($period =="LAST_YEAR"){		
				if ($this->is_hpos_enable){
					$query .= " AND YEAR(date_format( posts.date_created_gmt, '%Y-%m-%d')) = YEAR(DATE_SUB(date_format(CURDATE(), '%Y-%m-%d'), INTERVAL 1 YEAR)) "; 
				}else{
