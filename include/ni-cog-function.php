...
if( !class_exists( 'Ni_COG_Function' ) ) { 
	class Ni_COG_Function {
		...
		function is_hpos_enabled() {
			// Check if the function and class exist
			if (function_exists('wc_get_container') && class_exists('\Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController')) {
				...
				// Check if the container has the CustomOrdersTableController
				if ($container->has(\Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController::class)) {
					$hpos_controller = $container->get(\Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController::class);
					
					// Check if the HPOS feature is enabled
					return $hpos_controller->custom_orders_table_usage_is_enabled();
				}
			}
			
			return false; // HPOS is not enabled or the required function/class does not exist
		}
		...
		function get_sales_year_hpos(){
			...
			$query .= " Order By date_format( posts.date_created_gmt, '%Y')  DESC ";
